...
flowchart TD
    Q[学生提问] --> V{查询是否有效}
    V -- 无效 --> R0[(None, True) 交给上层]
    V -- 有效 --> C{Redis 精确缓存<br/>answer:query}
    C -- 命中 --> A1[直接返回答案]
    C -- 未命中 --> B[分词 + BM25 近似匹配<br/>语料=Redis 问题库]
    B --> T{相似度 > 阈值 0.85}
    T -- 是 --> M[取原始问题 → MySQL qa_knowledge 查答案]
    M -- 查到 --> W[写回 Redis 缓存] --> A1
    M -- 没查到 --> I
    T -- 否 --> I{意图识别<br/>BERT 分类}
    I -- 通用知识 --> L1[直接问 LLM，不检索]
    I -- 专业咨询 --> P[策略选择<br/>直接/HyDE/子查询/回溯问题<br/>改写查询]
    P --> S[Milvus 混合检索 + Reranker 重排<br/>多路结果合并去重]
    S --> L2[LLM 基于上下文+对话历史生成]
```
