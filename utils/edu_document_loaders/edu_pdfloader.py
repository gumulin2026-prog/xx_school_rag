...
import cv2
import pymupdf as fitz  # PyMuPDF（注意不是 pip install fitz）
import numpy as np
...
class OCRPDFLoader(BaseLoader):
    ...
    def pdf2text(self):
        # OCR 引擎惰性初始化：只有页面里真有大图片时才加载（纯文本 PDF 无需 rapidocr）
        ocr = None
        # 打开pdf文件
        ...
        resp = ""
        b_unit = tqdm(total=doc.page_count, desc="OCRPDFLoader context page index: 0", disable=True)
